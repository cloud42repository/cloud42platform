...
export const SCHEMA: Record<string, Record<string, EndpointSchema>> = {
  ...
  // ────────────────────────────────────────────────
  '/impossible-cloud': {
    // GET list columns (matched to DTO fields)
    'list-regions':                  { columns: ['name', 's3_url', 'iam_url', 'sts_url'] },
    'list-contracts':                { columns: ['id', 'distributorId', 'allocatedCapacity', 'reservedCapacity', 'currency'] },
    'list-contract-partners':        { columns: ['id', 'name', 'allocatedCapacity', 'allowOverdraft'] },
    'list-storage-accounts':         { columns: ['name', 'clientAccountId', 'contactEmail', 'allocatedCapacity', 'allowOverdraft'] },
    'get-storage-account':           { columns: ['name', 'clientAccountId', 'contactEmail', 'allocatedCapacity', 'allowOverdraft'] },
    'get-storage-accounts-usage':    { columns: ['accountId', 'usedStorage', 'quota', 'percentage'] },
    'get-storage-account-usage':     { columns: ['accountId', 'usedStorage', 'quota', 'percentage'] },
    'list-members':                  { columns: ['id', 'name', 'email', 'role'] },
    'get-partner':                   { columns: ['id', 'name', 'distributorContractId', 'allocatedCapacity', 'allowOverdraft'] },
    'list-partner-storage-accounts': { columns: ['name', 'clientAccountId', 'contactEmail', 'allocatedCapacity', 'allowOverdraft'] },
    'get-partner-storage-account':   { columns: ['name', 'clientAccountId', 'contactEmail', 'allocatedCapacity', 'allowOverdraft'] },
    'get-partner-storage-account-usage': { columns: ['accountId', 'usedStorage', 'quota', 'percentage'] },
    'get-partner-usage':             { columns: ['partnerId', 'usedStorage', 'quota', 'percentage'] },

    // POST / PUT / PATCH form fields (matched to DTO interfaces)
    'create-storage-account': { fields: [
      { key: 'name',               label: 'Account Name',          type: 'text',   required: true },
      { key: 'allocatedCapacity',  label: 'Allocated Capacity (GB)',type: 'number', required: true },
      { key: 'allowOverdraft',     label: 'Allow Overdraft',        type: 'select',
        options: [{ label: 'Yes', value: 'true' }, { label: 'No', value: 'false' }] },
      { key: 'contactEmail',       label: 'Contact Email',          type: 'email',  required: true,
        placeholder: 'Client account contact email' },
      { key: 'password',           label: 'Password',               type: 'text',   required: true,
        placeholder: 'Min 8 chars, upper/lower/digit/special' },
    ]},
    'patch-storage-account': { fields: [
      { key: 'pendingDeletedAt', label: 'Schedule Deletion (ISO-8601 or blank to cancel)', type: 'text',
        placeholder: '2026-12-31T00:00:00Z or leave empty to cancel' },
    ]},
    ...
    'create-partner': { fields: [
      { key: 'distributorContractId', label: 'Distributor Contract ID', type: 'text', required: true },
    ...
    'update-partner': { fields: [
      ...
      { key: 'allocatedCapacity', label: 'Allocated Capacity (GB)', type: 'number' },
    ]},
